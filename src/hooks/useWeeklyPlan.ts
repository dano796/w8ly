...
export function useWeeklyPlan() {
  const [plan, setPlan] = useLocalStorage<WeeklyPlan>('w8ly-weekly-plan', createEmptyPlan());
  ...
  const updateDayLabel = (day: DayName, label: string) => {
    setPlan(prev =>
      prev.map(d => (d.day === day ? { ...d, label } : d))
    );
  };
  ...
  const moveExercise = (fromDay: DayName, toDay: DayName, exerciseInstanceId: string) => {
    setPlan(prev => {
      const fromDayPlan = prev.find(d => d.day === fromDay);
      const exercise = fromDayPlan?.exercises.find(e => e.id === exerciseInstanceId);
      if (!exercise) return prev;
      return prev.map(d => {
        if (d.day === fromDay) return { ...d, exercises: d.exercises.filter(e => e.id !== exerciseInstanceId) };
        if (d.day === toDay) return { ...d, exercises: [...d.exercises, exercise] };
        return d;
      });
    });
  };
  ...
  return { plan, addExerciseToDay, removeExerciseFromDay, updateDayLabel, reorderExercises, moveExercise, resetPlan };
}
